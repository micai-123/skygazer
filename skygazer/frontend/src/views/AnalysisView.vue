      ...
      <div v-if="showAlertDetail" class="alert-modal-overlay" @click.self="closeAlertDetail">
        <div class="alert-modal">
          ...
          <div class="alert-modal-footer">
            ...
            <button class="modal-btn primary" @click="handleAlertAction(selectedAlert, 'subscribe')">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
...
import { SKYGAZER_THEME } from '@/utils/echartsTheme'
...
const initTempChart = () => {
  ...
  if (tempChartRef.value) {
    tempChart = echarts.init(tempChartRef.value, SKYGAZER_THEME)
  }
}
...
const initPrecipChart = () => {
  ...
  if (precipChartRef.value) {
    precipChart = echarts.init(precipChartRef.value, SKYGAZER_THEME)
  }
}
...
.stat-dot.blue { background: var(--signal); }
...
.alert-item.red {
  background: linear-gradient(90deg, rgba(239, 68, 68, 0.14) 0%, var(--surface-2) 35%);
}

.alert-item.orange {
  background: linear-gradient(90deg, rgba(249, 115, 22, 0.14) 0%, var(--surface-2) 35%);
}

.alert-item.yellow {
  background: linear-gradient(90deg, rgba(234, 179, 8, 0.14) 0%, var(--surface-2) 35%);
}

.alert-item.blue {
  background: linear-gradient(90deg, rgba(52, 227, 224, 0.14) 0%, var(--surface-2) 35%);
}
...
.alert-view-btn {
  padding: 0.5rem 1rem;
  background: var(--surface-2);
  border: 1px solid rgba(52, 227, 224, 0.3);
  border-radius: 8px;
  font-size: 0.8125rem;
  font-weight: 600;
  color: var(--blue-600);
...
.alert-view-btn:hover {
  ...
  box-shadow: 0 4px 12px rgba(52, 227, 224, 0.3);
}
