...
public class WeatherMapServiceImpl implements WeatherMapService {
    ...
    private JsonNode fetchFromAPI(String endpoint, String location) {
        // ========== 原和风天气API调用代码（已注释保留） ==========
        // API密钥为空时直接返回null，避免调用API
        // if (apiKey == null || apiKey.isEmpty()) {
        //     log.warn("和风天气API密钥未配置，跳过API调用");
        //     return null;
        // }
        // 
        // try {
        //     return retryUtil.executeWithRetryAndFallback(
        //         () -> {
        //             log.info("调用和风天气API: {}?location={}", endpoint, location);
        //             
        ...
        //                 if (!dataValidator.isValidWeatherResponse(root)) {
        //                     throw new RuntimeException("API返回无效响应: " + response);
        //                 }
        //                 
        //                 log.info("API调用成功: {}", endpoint);
        //                 return root;
        //             } catch (com.fasterxml.jackson.core.JsonProcessingException e) {
        //                 throw new RuntimeException("JSON解析失败", e);
        //             }
        //         },
        //         "获取天气数据-" + endpoint + "-" + location,
        //         () -> null
        //     );
        // } catch (Exception e) {
        //     log.error("API调用失败 {}: {}", endpoint, e.getMessage(), e);
        //     return null;
        // }
        // ========== 原和风天气API调用代码结束 ==========
        
        // 替代实现：不再调用和风天气API，直接返回null，使用模拟数据
        log.info("跳过和风天气API调用，使用数据库或模拟数据: {}?location={}", endpoint, location);
        return null;
    }
