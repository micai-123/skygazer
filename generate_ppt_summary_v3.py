...
def create_ppt_chapter_summary():
    ...
    for i, tech in enumerate(frontend_items):
        x_pos = 22 + i * 11
        circle = Circle((x_pos, 26), 4, facecolor='#4ECDC4', edgecolor='white', linewidth=1.5)
        ax.add_patch(circle)
    ...
    for i, tech in enumerate(backend_items):
        x_pos = 22 + i * 11
        circle = Circle((x_pos, 18), 4, facecolor='#FF6B6B', edgecolor='white', linewidth=1.5)
        ax.add_patch(circle)
        ax.text(x_pos, 18, tech, ha='center', va='center',
                fontsize=13, fontweight='bold', color='#1A1A2E')
    
    # 设计原则
    ax.text(85, 27, '设计原则', ha='right', va='center',
    ...
    for i, principle in enumerate(principles):
        ax.text(65, 24.5 - i * 3.5, f'• {principle}', ha='left', va='center',
                fontsize=13, color='#E9E3E6')
