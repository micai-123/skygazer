...
import com.skygazer.weather.dto.geo.GeoLookupResponse;
...
public class GeoLookupServiceImpl implements GeoLookupService {
    ...
    public List<CityLocation> searchCities(String location, String adm, String range, int number) {
        try {
            ...
            GeoLookupResponse response = geoWebClient.get()
                .uri(uriBuilder -> {
                    uriBuilder.path("/city/lookup")
                        .queryParam("location", location)
                        .queryParam("key", apiKey)
                        .queryParam("number", number);
                    
                    if (adm != null && !adm.isEmpty()) {
                        uriBuilder.queryParam("adm", adm);
                    }
            ...
            if (response != null && response.isSuccess() && response.getLocation() != null) {
                List<CityLocation> cities = response.getLocation();
    ...
    public List<CityLocation> searchCitiesByProvince(String provinceName) {
        ...
        List<CityLocation> cities = searchCities(keyword, null, "cn", 20);
        
        List<CityLocation> filteredCities = cities.stream()
            .filter(city -> {
        ...
        if (!filteredCities.isEmpty()) {
            provinceCitiesCache.put(provinceName, filteredCities);
        }
    ...
    public CityLocation getCityByName(String cityName) {
        ...
        List<CityLocation> cities = searchCities(cityName, null, "cn", 1);
        if (!cities.isEmpty()) {
            CityLocation city = cities.get(0);
