...
const normalizeCityName = (name, geoJsonFeatures) => {
  if (!name) return name
  
  const geoNames = geoJsonFeatures?.map(f => f.properties?.name || f.properties?.namecn || '') || []
  
  if (geoNames.includes(name)) {
    return name
  }
  
  if (geoNames.includes(name + '市')) {
    return name + '市'
  }
  
  if (geoNames.includes(name + '地区')) {
    return name + '地区'
  }
  
  if (name.endsWith('市') && geoNames.includes(name.slice(0, -1))) {
    return name.slice(0, -1)
  }
  
  const matchedGeo = geoNames.find(geoName => 
    geoName.includes(name) || name.includes(geoName)
  )
  
  return matchedGeo || name
}
...
const getChartOption = (geoJson, data) => {
  ...
  const mapData = isProvinceView 
    ? data.map(city => ({
        name: normalizeCityName(city.name, geoFeatures),
        value: city.value,
  ...
  console.log('🗺️ 数据名称标准化:', {
    original: data.slice(0, 3).map(c => c.name),
    normalized: mapData.slice(0, 3).map(m => m.name),
    geoNames: geoFeatures.slice(0, 5).map(f => f.properties?.name)
  })
        ...
        if (params.componentType === 'series') {
          if (isProvinceView) {
            const city = data.find(c => 
              c.name === params.name || 
              c.name + '市' === params.name || 
              (params.name.endsWith('市') && c.name === params.name.slice(0, -1)) ||
              normalizeCityName(c.name, geoFeatures) === params.name
            )
            if (city) {
              const config = currentLayerConfig.value
